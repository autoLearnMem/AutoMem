...
AUTO_PRUNE_MAX_LINES = 200
AUTO_PRUNE_KEEP_LINES = 100
STRATEGY_PRUNE_MAX = 60
STRATEGY_PRUNE_KEEP = 25
MONSTER_PRUNE_MAX = 150
MONSTER_PRUNE_KEEP = 50
...
_OPPOSITE_DIRECTIONS = {
    ...
    "far north": "far south", "far south": "far north",
    "far east": "far west", "far west": "far east",
    "far northeast": "far southwest", "far southwest": "far northeast",
    "far northwest": "far southeast", "far southeast": "far northwest",
}
...
YES/NO PROMPTS: When the game asks a yes/no question like "Eat it? [yn]" or "Continue eating? [yn]", respond with just 'y' or 'n'.
...
DISMISSING MENUS AND PROMPTS: When the game shows a menu, list, or "--More--" prompt, you MUST dismiss it before doing anything else. Output 'esc' to close/cancel a menu, or 'space' to continue past a "--More--" prompt. If your actions seem to have no effect and the observation is not changing, you are likely stuck in a menu -- try 'esc' or 'space' to dismiss it.

CORPSE SAFETY: Do NOT eat old or rotten corpses -- they can poison and KILL you! Only eat fresh corpses (from monsters you just killed this turn or last turn). When the game asks about eating a corpse, say 'n' if you are unsure or if several turns have passed since it died. Safe foods from your inventory (food rations, apples, oranges, lembas wafers) are always preferable. NEVER eat while Satiated -- you will choke and die!

AVOID QUITTING: Do NOT go up stairs ('<') at Dungeon Level 1 -- this exits the game. Do NOT use the 'quit' command. Your goal is to survive and progress deeper.
"""
...
NLE_INITIAL_STRATEGY = """\
=== STRATEGIC GOALS ===
PRIMARY: Find stairs down '>' and DESCEND to the next dungeon level.
  - Explore systematically: pick a direction, follow corridors, check all rooms.
  - When you find '>' stairs, stand on them and output 'down' IMMEDIATELY.
  - Do NOT delay descending to explore more or pick up items.
  - At Dlvl:1, the '<' stairs lead OUT of the game. NEVER go up at Dlvl:1.

SURVIVAL RULES:
  - Eat when Hungry (use safe food from inventory, NOT old corpses).
  - NEVER eat while Satiated (instant death by choking).
  - NEVER eat rotten/old corpses (food poisoning death).
  - If HP < 15 and haven't prayed in 300+ turns, pray.
  - Avoid floating eyes (letter 'e') in melee (paralysis).
  - Output 'esc' if stuck in a menu.

=== NAVIGATION NOTES ===
(Append stair locations, shop locations, important landmarks only)

=== LESSONS LEARNED ===
(Append only important lessons about game mechanics)
"""
...
MENU / PROMPT RESPONSES: esc (exit/dismiss menu), minus, space (continue past --More--), apos, y, n, 0-9 (for responding to in-game prompts). Also single letters a-z for item selection when prompted.
...
- DISMISSING MENUS: If you see a menu/list/inventory screen or your actions have no effect, output 'esc' to dismiss it. Output 'space' to continue past "--More--" prompts.
- CORPSE SAFETY: Do NOT eat old/rotten corpses -- they POISON and KILL you! Only eat fresh corpses or safe food from inventory. NEVER eat while Satiated -- you will CHOKE and DIE!
- Do NOT go up '<' at Dlvl:1 -- this EXITS the game and ends your run.
...
- Check strategy.txt for your current goals, safety rules, and lessons learned.
- The system auto-updates current_status.txt and inventory.txt each step for your reference.
...
- Read the status line every turn: watch HP, hunger status, and conditions. If Hungry or Weak, eat. If HP is low, consider retreat or healing. If you see FoodPois or Ill, use healing potions or pray immediately!
- Check your dungeon level (Dlvl). At Dlvl:1, going UP exits the dungeon and ends the game. Always go DOWN to progress.
- NEVER eat when the status line says "Satiated" -- you will choke and die.

YOUR PRIMARY GOAL: Find stairs down '>' and descend as deep as possible. When you find '>' stairs, go to them and use 'down' IMMEDIATELY. Do not delay.
...
- DISMISSING MENUS: If your actions have no effect, output 'esc' to dismiss any menu.
- Consult game_rules.txt in your memory files for detailed action format rules.
...
INITIAL_FILES_BY_ENV = {
    ...
    "nle": {
        ...
        "inventory.txt": "",
        "actions_log.txt": "",
        "strategy.txt": NLE_INITIAL_STRATEGY,
        "current_status.txt": "",
    },
    "minihack": {
        "game_rules.txt": NLE_GAME_RULES,
...
SPATIAL_GUIDANCE = {
    ...
    "nle": (
        ...
        "walls ('-', '|') or solid rock (space). Use UPSERT_MAP to record STATIC "
        "features (walls, stairs, doors, corridors) by coordinate -- this prevents "
        "duplicate entries. Format: <|UPSERT_MAP|>x,y|description<|END|>.\n\n"
        "DYNAMIC ENTITIES: Monsters and pets MOVE every turn. Do NOT record their "
        "positions in dungeon_map.txt -- they will be wrong next turn. Only record "
        "STATIC terrain features. Use monster_encounters.txt for combat notes.\n\n"
        ...
        "retreat, healing, or prayer), FoodPois/Ill (use healing potion or pray "
        "IMMEDIATELY), and Satiated (do NOT eat -- you will choke!).\n\n"
        ...
        "move into its tile.\n\n"
        "MENU ESCAPE: If the observation is not changing despite your actions, "
        "you may be stuck in a menu or prompt. Output 'esc' to dismiss it."
    ),
    "minihack": (
...
AVAILABLE COMMANDS:
  ...
  <|UPSERT_MAP|>x,y|description<|END|>        Update/insert a dungeon_map.txt entry for coordinate (x,y).
                                               This REPLACES any existing entry for (x,y), preventing duplicates.
                                               Example: <|UPSERT_MAP|>42,4|floor, stairs_down to south, corridor to east<|END|>
  <|DONE|>                                     Finish logging, move on to action selection

MAP LOGGING RULES:
- Use UPSERT_MAP (not APPEND) for dungeon_map.txt to avoid duplicate entries per coordinate.
- Only record STATIC terrain (walls, floors, doors, stairs, corridors). Do NOT record monster or pet positions in the map -- they move every turn.
- Use the coordinate (x,y) from the observation's cursor/player position.
- If you discover stairs down '>': ALWAYS record it in dungeon_map.txt AND append to strategy.txt under NAVIGATION NOTES.

STRATEGY.TXT RULES -- IMPORTANT:
- strategy.txt is for IMPORTANT STRATEGIC INSIGHTS ONLY.
- ONLY append when you learn something truly new and important:
  * Found stairs down or up (with approximate coordinates)
  * Discovered a dangerous enemy or deadly trap
  * Learned a game mechanic lesson (e.g., "eating old corpses kills you")
  * Major strategy change needed
- Do NOT append routine observations like "moved east", "saw a corridor", "pet is nearby".
- Do NOT log every step -- this makes strategy.txt too large to be useful.

MONSTER ENCOUNTERS:
- Only log HOSTILE monster encounters and combat results.
- Do NOT log tame pet sightings -- they add noise and no value.

LOGGING EFFICIENCY:
- If your action was a routine movement and NOTHING noteworthy happened (no combat, no items found, no discoveries, no status changes, no new rooms), output <|DONE|> immediately.
- Not everything needs logging -- focus on events that will help FUTURE decision-making.

IMPORTANT RULES:
1. BEFORE appending to ANY file (except dungeon_map.txt -- use UPSERT_MAP instead), first SEARCH or READ it to check whether the information is already recorded. Do NOT duplicate entries.
2. When logging, format: "Step N action 'X': outcome description"
3. DO log: failed/invalid actions, UNCHANGED observations, discoveries (stairs, items, doors), combat, damage, inventory changes, status conditions, DANGER events.
4. DO NOT log: routine movements with no events, pet/tame animal positions.
...
Continue logging (remember: SEARCH before writing to avoid duplicates, use UPSERT_MAP for map entries), or output <|DONE|> to move on to action selection."""

PLAN_SYSTEM = """\
{stairs_urgency}{instruction_prompt}

You have an external memory system to help you make better decisions.
...
1. Check the GAME STATUS above for your HP, dungeon level, hunger, and conditions.
2. Address URGENT needs FIRST -- in this priority order:
   a. If stairs down are adjacent or you are standing on them: GO DOWN NOW. Output 'down' if on stairs, or move toward them. This is your TOP PRIORITY.
   b. If FoodPois or Ill: pray or quaff healing potion IMMEDIATELY.
   c. If HP critically low (<15): pray (if >300 turns since last prayer) or quaff healing potion.
   d. If Hungry/Weak/Fainting: eat safe food (inventory food, NOT old corpses).
   e. If Satiated: do NOT eat anything.
3. If no urgency, plan your action. Your primary goal is to FIND and DESCEND stairs down '>'.
   - If you know where stairs down are (check MAP ANALYSIS), navigate toward them.
   - If you don't know where stairs are, explore systematically: follow corridors, check rooms.
   - Do NOT wander aimlessly. Pick a direction and commit to it until you hit a wall or find something.
4. BEFORE committing, verify your action is valid:
   - Check MAP ANALYSIS above for passable/blocked directions (it is accurate).
   - If using an item, check game_rules.txt for the correct format.
5. When confident, commit: <|ACTION|>your_chosen_action<|END|>

AVAILABLE COMMANDS (read-only -- no writing):
  <|SEARCH|>filename.txt|keyword<|END|>    Search for lines matching keyword
  <|READ|>filename.txt<|END|>              Read entire file
  <|TAIL|>filename.txt|N<|END|>            Read last N lines
  <|COUNT|>filename.txt|keyword<|END|>     Count matching lines
  <|ACTION|>your_chosen_action<|END|>      Commit to an action

You MUST eventually output exactly one <|ACTION|> to commit.{stairs_reminder}"""
...
class MemoryFileSystem:
    ...
    def upsert_map(self, x, y, description):
        filename = "dungeon_map.txt"
        content = self._read_raw(filename) or ""
        key = "({},{})".format(x, y)
        new_line = "{}: {}".format(key, description)

        lines = content.splitlines()
        filtered = [l for l in lines if not l.startswith(key + ":")]
        filtered.append(new_line)

        self._write_raw(filename, "\n".join(filtered) + "\n")
        return "[Updated map entry for {}]".format(key)

    def auto_prune(self, filename, max_lines=AUTO_PRUNE_MAX_LINES,
                   keep_lines=AUTO_PRUNE_KEEP_LINES):
        filename = self._safe_name(filename)
        if not self._exists(filename):
            return
        content = self._read_raw(filename) or ""
        ...
        if len(lines) <= max_lines:
            return
        pruned_count = len(lines) - keep_lines
        kept = lines[-keep_lines:]
        header = "[... {} older entries pruned at step boundary ...]".format(pruned_count)
        self._write_raw(filename, header + "\n" + "\n".join(kept) + "\n")
        logger.info("Auto-pruned %s: removed %d lines, kept %d",
                     filename, pruned_count, keep_lines)
...
def parse_status_line(obs_text):
    info = {}

    m = re.search(r'Dlvl:(\d+)', obs_text)
    if m:
        info['dlvl'] = int(m.group(1))

    m = re.search(r'HP:(\d+)\((\d+)\)', obs_text)
    if m:
        info['hp'] = int(m.group(1))
        info['hp_max'] = int(m.group(2))

    m = re.search(r'Xp:(\d+)', obs_text)
    if m:
        info['xp_level'] = int(m.group(1))

    m = re.search(r'\$:(\d+)', obs_text)
    if m:
        info['gold'] = int(m.group(1))

    for status in ['Satiated', 'Hungry', 'Weak', 'Fainting']:
        if re.search(r'\b' + status + r'\b', obs_text):
            info['hunger'] = status
            break
    if 'hunger' not in info:
        info['hunger'] = 'Normal'

    conditions = []
    for cond in ['FoodPois', 'Ill', 'Conf', 'Stun', 'Hallu', 'Blind']:
        if cond in obs_text:
            conditions.append(cond)
    info['conditions'] = conditions

    m = re.search(r'[Pp]osition[:\s]*\(?(\d+)[,\s]+(\d+)\)?', obs_text)
    if not m:
        m = re.search(r'[Cc]ursor[:\s]*\(?(\d+)[,\s]+(\d+)\)?', obs_text)
    if m:
        info['x'] = int(m.group(1))
        info['y'] = int(m.group(2))

    return info

def detect_stairs_down(obs_text, map_analysis):
    obs_lower = obs_text.lower()
    on_stairs = ("staircase down here" in obs_lower or
                 "stairway down here" in obs_lower)

    adjacent_stairs = False
    stairs_direction = None
    if map_analysis:
        map_upper = map_analysis.upper()
        
        if "STAIRS ADJACENT" in map_upper:
            for line in map_analysis.splitlines():
                line_upper = line.upper()
                if "STAIRS" in line_upper and ("DOWN" in line_upper or "'>'" in line):
                    for d in _ALL_DIRECTIONS:
                        if d in line.lower():
                            adjacent_stairs = True
                            stairs_direction = d
                            break
                    if stairs_direction:
                        break

    return on_stairs, adjacent_stairs, stairs_direction

def detect_oscillation(recent_actions):
    if len(recent_actions) < 4:
        return False, ""

    movements = [a for a in recent_actions[-10:] if a in _MOVEMENT_ACTIONS]
    if len(movements) < 4:
        return False, ""

    reversals = 0
    for i in range(1, len(movements)):
        if _OPPOSITE_DIRECTIONS.get(movements[i]) == movements[i - 1]:
            reversals += 1

    if reversals >= 3:
        recent_str = " -> ".join(movements[-6:])
        return True, ("WARNING: You are OSCILLATING between opposite directions (reversed {} times in last {} moves: {}). STOP repeating this pattern. Try something COMPLETELY DIFFERENT: 'search' for hidden passages, 'open' a nearby door, try a diagonal direction (northeast, southeast, etc.), or explore a corridor you have not visited.").format(reversals, len(movements), recent_str)
...
def parse_log_calls(text):
    ...
    for m in re.finditer(_OPEN["UPSERT_MAP"] + r"(.*?)" + _END, text, re.DOTALL):
        parts = m.group(1).split("|", 1)
        if len(parts) == 2 and parts[0].strip() and parts[1].strip():
...
class MemoryAgent(BaseAgent):
    ...
    def act(self, obs, prev_action=None, prev_validated_action=None):
        ...
        if self._prev_obs_text is not None:
            if obs_text.strip() == self._prev_obs_text.strip():
                ...
                if self._unchanged_count >= 3:
                    diff_notice = ("WARNING: The observation has been UNCHANGED for {} consecutive actions. You are STUCK. Your recent actions have had NO EFFECT on the environment. You MUST try a COMPLETELY DIFFERENT approach -- do not repeat any of your recent actions. Try 'esc' to dismiss any menu, or 'space' to continue past a --More-- prompt, or try a direction you have not tried recently.").format(self._unchanged_count)
                else:
                    diff_notice = ("WARNING: The observation is IDENTICAL to the previous step. Your previous action likely had NO EFFECT. This is important to log -- the action may be invalid or inapplicable in the current state. Try 'esc' if you might be in a menu.")
            else:
                self._unchanged_count = 0
        ...
        if self._env_name == "nle":
            status_info = parse_status_line(obs_text)
            if 'x' in status_info and 'y' in status_info:
                self._visited_positions.add((status_info['x'], status_info['y']))
        ...
        if prev_action is not None and self.step_count > 0:
            ...
            self.memory.auto_prune("actions_log.txt")
            self.memory.auto_prune("strategy.txt",
                                   max_lines=STRATEGY_PRUNE_MAX,
                                   keep_lines=STRATEGY_PRUNE_KEEP)
            self.memory.auto_prune("monster_encounters.txt",
                                   max_lines=MONSTER_PRUNE_MAX,
                                   keep_lines=MONSTER_PRUNE_KEEP)

        if self._env_name in ("nle", "minihack"):
            self._update_current_status(obs_text)
            self._update_inventory(obs)

        on_stairs, adjacent_stairs, stairs_direction = detect_stairs_down(
            obs_text, map_analysis)

        stairs_urgency = ""
        stairs_reminder = ""
        if on_stairs:
            stairs_urgency = (
                "*** CRITICAL PRIORITY: YOU ARE STANDING ON STAIRS DOWN ('>')! ***\n"
                "You MUST output <|ACTION|>down<|END|> to descend to the next level.\n"
                "Descending is ALWAYS the right choice -- it is your PRIMARY OBJECTIVE.\n"
                "Do NOT explore, do NOT pick up items, do NOT fight. Just go DOWN.\n\n"
            )
            stairs_reminder = (
                "\n\nREMINDER: You are on stairs down ('>'). "
                "Output <|ACTION|>down<|END|> NOW. Do not do anything else."
            )
        elif adjacent_stairs and stairs_direction:
            stairs_urgency = (
                "*** HIGH PRIORITY: STAIRS DOWN ARE 1 TILE {} ***\n"
                "Move {} to reach the stairs, then use 'down' to descend.\n"
                "This is your most important goal. Do not get distracted.\n\n"
            ).format(stairs_direction.upper(), stairs_direction)
            stairs_reminder = (
                "\n\nREMINDER: Stairs down are {} of you. Go {} NOW."
            ).format(stairs_direction, stairs_direction)

        is_oscillating, oscillation_warning = detect_oscillation(
            self._recent_actions)

        game_status = self._build_game_status(obs_text)
        ...
        plan_debug = self._plan_validate_phase(
            ...
            stairs_urgency=stairs_urgency,
            stairs_reminder=stairs_reminder,
        ...
        effective_action = final_action.strip().lower()
        self._recent_actions.append(effective_action)
    ...
    def _execute_write_op(self, op, args):
        if op == "READ":
            ...
            return self.memory.upsert_map(args["x"], args["y"], args["description"])
    ...
    def _format_tool_result(self, op, args, result):
        if op == "READ":
            ...
            header = "UPSERT_MAP ({},{})".format(args["x"], args["y"])
            return header, "-- {} --\n{}".format(header, result)
        else:
            return result, result
        return header, "-- {} --\n{}".format(header, result)

    def _build_game_status(self, obs_text):
        if self._env_name != "nle":
            return ""

        status = parse_status_line(obs_text)
        if not status.get('dlvl'):
            return ""

        parts = ["=== GAME STATUS ==="]

        line_parts = []
        line_parts.append("Dlvl:{}".format(status.get('dlvl', '?')))
        if 'hp' in status:
            hp_pct = status['hp'] * 100 // status['hp_max'] if status['hp_max'] > 0 else 0
            line_parts.append("HP:{}/{} ({}%)".format(
                status['hp'], status['hp_max'], hp_pct))
        line_parts.append("Hunger:{}".format(status.get('hunger', '?')))
        if 'xp_level' in status:
            line_parts.append("Xp:{}".format(status['xp_level']))
        line_parts.append("Step:{}".format(self.step_count))
        parts.append(" | ".join(line_parts))

        if status.get('conditions'):
            parts.append("!!! CONDITIONS: {} -- address immediately !!!".format(
                ", ".join(status['conditions'])))

        if self._recent_actions:
            recent = self._recent_actions[-6:]
            parts.append("Recent actions: {}".format(", ".join(recent)))

        parts.append("Tiles explored: {}".format(len(self._visited_positions)))

        return "\n".join(parts)

    def _update_current_status(self, obs_text):
        status = parse_status_line(obs_text)

        lines = ["=== CURRENT GAME STATUS (auto-updated each step) ==="]
        lines.append("Dungeon Level: {}".format(status.get('dlvl', 'Unknown')))
        if 'hp' in status:
            lines.append("HP: {}/{}".format(status['hp'], status['hp_max']))
        lines.append("Hunger: {}".format(status.get('hunger', 'Unknown')))
        if status.get('conditions'):
            lines.append("!!! CONDITIONS: {} !!!".format(
                ", ".join(status['conditions'])))
        if 'xp_level' in status:
            lines.append("Experience Level: {}".format(status['xp_level']))
        if 'gold' in status:
            lines.append("Gold: {}".format(status['gold']))
        if 'x' in status and 'y' in status:
            lines.append("Position: ({},{})".format(status['x'], status['y']))
        lines.append("Step: {}".format(self.step_count))
        lines.append("Tiles explored: {}".format(len(self._visited_positions)))
        ...
        self.memory.write("current_status.txt", "\n".join(lines))

    def _update_inventory(self, obs):
        short_ctx = obs.get("text", {}).get("short_term_context", "")
        if short_ctx and short_ctx.strip():
            self.memory.write("inventory.txt",
                              "=== CURRENT INVENTORY (auto-updated) ===\n"
                              + short_ctx.strip())
    ...
    def _plan_validate_phase(self, obs_text, instruction_prompt,
                             ...
                             stairs_urgency="", stairs_reminder="",
                             oscillation_warning="", game_status=""):
        file_index = self.memory.file_index()
        ...
        for iteration in range(self.max_validate_iterations):
            ...
            if not calls:
                logger.warning(
                    "Plan gameplay: no SEARCH/READ/TAIL/COUNT/ACTION found. Raw: %s", assistant_text[:200],
                )
        ...
        if final_action is None:
            ...
            logger.warning(
                "Plan phase exhausted iterations without <|ACTION|>. Defaulting to '%s'.", fallback,
            )
